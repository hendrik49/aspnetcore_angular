...
namespace angular_netcore.Repository
{
  public class DbSeeder
  {
    ...
    public async Task SeedAsync(IServiceProvider serviceProvider)
    {
      ...
      {
        var customersDb = serviceScope.ServiceProvider.GetService<AngularNetCoreDbContext>();
    ...
    public async Task InsertCustomersSampleData(AngularNetCoreDbContext db)
    {
      ...
      {
        int numAffected = await db.SaveChangesAsync();
        _Logger.LogInformation(@"Saved {numAffected} tasks");
      }
      catch (Exception exp)
      {
        _Logger.LogError($"Error in {nameof(DbSeeder)}: " + exp.Message);
    ...
    private List<Tasks> GetTasks()
    {
      var tasks = new List<Tasks>
            {
                new Tasks { name = "Alabama", when = DateTime.Now, status=true },
                new Tasks { name = "Montana",when = DateTime.Now.AddDays(1),status=true },
                new Tasks { name = "Alaska", when = DateTime.Now.AddDays(2),status=false },
                new Tasks { name = "Nebraska", when = DateTime.Now.AddDays(3),status=false },
